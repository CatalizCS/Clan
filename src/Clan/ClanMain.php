...
class ClanMain extends PluginBase implements Listener {
    ...
    public function onEnable() {
        ...
        if (!file_exists($this->getDataFolder() . "BannedNames.txt")) {
            ...
            $txt = "Admin:admin:Staff:staff:Owner:owner:Builder:builder:Op:OP:op:POLICE:police:Police";
            fwrite($file, $txt);
        }
        ...
        if (!$this->antispam) {
            $this->getLogger()->info("Thêm AntiSpamPro để ban những tên trong danh sách cấm");
        }
        $this->purechat = $this->getServer()->getPluginManager()->getPlugin("PureChat");
        if (!$this->purechat) {
            $this->getLogger()->info("Thêm PureChat để hiển thị thông tin lên tên");
        }
    ...
    public function sendListOfTop10FactionsTo($s) {
        ...
        $i = 0;
        $s->sendMessage($this->formatMessage("~ Top 10 Clan Mạnh Nhất ~", true));
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            $j = $i + 1;
        ...
        if ($this->cornerIsInPlot($x + $arm, $z + $arm, $x - $arm, $z - $arm, $level->getName())) {
            ...
            if ($this->prefs->get("EnableOverClaim")) {
                if ($power_sender < $power_claimedBy) {
                    $sender->sendMessage($this->formatMessage("Khu này đã được chiếm bởi $claimedBy với $power_claimedBy STR. Clan của bạn có $power_sender Năng Lượng. Bạn không có đủ năng lượng để chiếm khu này từ người khác!"));
                } else {
                    $sender->sendMessage($this->formatMessage("Khu này được chiếm bởi $claimedBy với $power_claimedBy STR. Clan của bạn có $power_sender Năng lượng. Nhập /clan overclaim để chiếm lấy khu đất này nếu bạn muốn."));
                }
                return false;
            } else {
                $sender->sendMessage($this->formatMessage("Overclaiming Đã bị tắt."));
                return false;
            }
        }
