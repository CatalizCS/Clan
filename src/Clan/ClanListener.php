    ...
    public function factionChat(PlayerChatEvent $PCE) {
        ...
        if ($this->plugin->motdWaiting($player)) {
            if (time() - $this->plugin->getMOTDTime($player) > 30) {
                $PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Thời Gian Đã Hết, vui lòng hãy sử dụng /clan desc lại lần nữa."));
                ...
                $PCE->setCancelled(true);
                $PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Đã thay đổi thông tin clan thành công, nhập /clan info để xem chi tiết.", true));
            }
            return true;
        }
    ...
    public function factionBlockBreakProtect(BlockBreakEvent $event) {
        ...
        if ($this->plugin->pointIsInPlot($x, $z, $level)) {
            if ($this->plugin->factionFromPoint($x, $z, $level) === $this->plugin->getFaction($event->getPlayer()->getName())) {
                ...
                $event->getPlayer()->sendMessage($this->plugin->formatMessage("Bạn không thể đập các khối ở đây, khu đất này đã có clan khác, vui lòng nhập /clan plotinfo để biết thêm chi tiết về plot này."));
                return;
            }
        }
    }
    ...
    public function factionBlockPlaceProtect(BlockPlaceEvent $event) {
        ...
        if ($this->plugin->pointIsInPlot($x, $z, $level)) {
            if ($this->plugin->factionFromPoint($x, $z, $level) == $this->plugin->getFaction($event->getPlayer()->getName())) {
                ...
                $event->setCancelled(true);
                $event->getPlayer()->sendMessage($this->plugin->formatMessage("Bạn không thể đặt các khối ở đây, khu đất này đã có clan khác, vui lòng nhập /clan plotinfo để biết thêm chi tiết về plot này."));
                return;
            }
        }
    }
